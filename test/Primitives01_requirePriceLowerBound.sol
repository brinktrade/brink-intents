...
contract Primitives01_requirePriceLowerBound is Test, Helper  {
  ...
  function testRequirePriceLowerBound_priceIsZero () public {
    ...
    primitives.requirePriceLowerBound(mockPriceOracle, abi.encode(0), 500);
  }

  // when oraclePrice is below lowerBoundPrice, should not revert
  function testRequirePriceLowerBound_priceIsBelow () public {
    primitives.requirePriceLowerBound(twapAdapter, abi.encode(address(USDC_ETH_FEE500_UNISWAP_V3_POOL), uint32(1000)), MAGIC_TWAP_PRICE_USDC_ETH_1000_0 + 1);
  }

  // when oraclePrice is equal to lowerBoundPrice, should not revert
  function testRequirePriceLowerBound_priceIsEqual () public {
    primitives.requirePriceLowerBound(twapAdapter, abi.encode(address(USDC_ETH_FEE500_UNISWAP_V3_POOL), uint32(1000)), MAGIC_TWAP_PRICE_USDC_ETH_1000_0);
  }
